...
import pprint
import argparse
from masked_mnist import MultiMaskMNIST
...
class MMNet(nn.Module):              
    ...
    def forward(self, x1, x2):      # define the network using the module in __init__
        ...
        feat = torch.mean(torch.stack([x1, x2]), 0)
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.data = MultiMaskMNIST(m1root = args.mnist_path, m2root = args.fashion_path, m1_block_len=args.m_block_len, m2_block_len = args.f_block_len, block_len_max = args.block_len_max, fix_seed = args.fix_seed, train=True)
        
        self.val_data = MultiMaskMNIST(m1root = args.mnist_path, m2root = args.fashion_path, m1_block_len=args.m_block_len, m2_block_len = args.f_block_len,block_len_max = args.block_len_max, fix_seed = args.fix_seed, train=True)
        ...
        self.data_loader = DataLoader(self.data, batch_size=args.batch_size,sampler=train_sampler, drop_last=True)
        
        self.mask_loader = DataLoader(self.data, batch_size=args.batch_size, sampler=train_sampler,drop_last=True)
        
        self.val_loader = DataLoader(self.val_data, batch_size=args.batch_size,sampler=valid_sampler,  drop_last=False)
        
        self.test_data = MultiMaskMNIST(m1root = args.mnist_path, m2root = args.fashion_path, m1_block_len=args.m_block_len, m2_block_len = args.f_block_len, block_len_max = args.block_len_max, fix_seed = args.fix_seed, train=False)
            ...
            if epoch % self.args.plot_interval == 0:
                with torch.no_grad():
                    ...
                    if self.ae_weight > 0:
                        ...
                        test_feat, out = self.multi_cls(m_x_rec.detach(), f_x_rec.detach())
            
                    else:
                        test_feat = None

                    z, z_gen, x_rec, x_gen, ae_loss = self.m_pbigan(test_x1 * test_mask1, test_mask1, test_feat)
                    fz, fz_gen, fx_rec, fx_gen, fae_loss = self.f_pbigan(test_x2 * test_mask2, test_mask2, test_feat)
                    if self.args.keepobsv:
                        x_rec = x_rec * (1 - test_mask1) + test_x1 * test_mask1
                        fx_rec = fx_rec * (1 - test_mask2) + test_x2 * test_mask2
                        
                    _, test_out = self.multi_cls(x_rec.detach(), fx_rec.detach())
    ...
    def train_epoch(self, epoch):
        ...
        for (xlist, masklist, _, label), (_, mask_genlist, _, _) in zip(self.data_loader, self.mask_loader):
            ...
            m_rec, gen_m_pred = self.train_m_pbigan(x1, mask1, mask_gen1, label, x2, mask2)
            f_rec, gen_f_pred = self.train_f_pbigan(x2, mask2, mask_gen2, label, x1, mask1) 
...
#             self.cycle_f_pbigan(x1, mask1, label, x2, mask2)   # update the generator of f_pbigan
            if epoch > self.ae_flat:
                ...
                if self.args.trclsori:
                    feat, pred = self.multi_cls(x1, x2) 
                else:
                    feat, pred = self.multi_cls(m_rec, f_rec)     
                loss = min(5 * self.ae_weight, 1) * nn.CrossEntropyLoss()(pred, label) 
                ...
                m_pred_cls = gen_m_pred.data.max(1)[1]
                f_pred_cls = gen_f_pred.data.max(1)[1]
    ...
    def val_epoch(self, epoch, phase):
        ...
        for batch_idx, (xlist, masklist, _, label) in enumerate(loader):
            ...
            if epoch > self.ae_flat:
                ...
                _, m_pred = self.m_net(m_rec)
                _, f_pred = self.f_net(f_rec)
                
                pred_cls = pred.data.max(1)[1]
                m_pred_cls = m_pred.data.max(1)[1]
                f_pred_cls = f_pred.data.max(1)[1]
    ...
    def test_epoch(self, epoch):
        ...
        self.m_net.eval()
        self.m_critic.eval()
        models = torch.load('/nfs/masi/gaor2/saved_file/MissClinic/MNIST2Fashion/MissGAN/crossM_cls/fashion-pbigan/crossM_1229.100411_block_10_20_None/model/0199.pth')
        ...
        for batch_idx, (xlist, masklist, _, label) in enumerate(self.data_loader):
            ...
            x1, x2 = x1.to(device), x2.to(device)
            mask1, mask2 = mask1.to(device).float(), mask2.to(device).float() 
            m_rec = self.train_m_pbigan(x1, mask1, mask1, label,  x2) # response to x1
            f_rec = self.train_f_pbigan(x2, mask2, mask2, label, x1) # response to x2
            
            if epoch > self.ae_flat + 50:
                pred = self.multi_cls(m_rec, f_rec)             # here should be careful
                #loss = nn.CrossEntropyLoss()(pred, label) 
    ...
    def cycle_m_pbigan(self, x, mask, label, another_x, another_mask):
        ...
        ### generate feat
        if self.ae_weight > 0:
            feat, out = self.f_net(another_x * another_mask)
        else:
            feat = None
        
        _, _, m_rec, _, _ = self.m_pbigan(x * mask, mask, feat, ae=False)    # generate m, x is fashion
        
        if self.ae_weight > 0:
            feat, out = self.m_net(m_rec)
        else:
            feat = None
        
        z_enc, z_gen, x_rec, x_gen, ae_loss = self.f_pbigan(another_x * another_mask, another_mask, feat, ae=True) # generate f
        
        cycle_loss = 0
        
        if self.ae_weight > 0:
            gen_feat, gen_out = self.f_net(x_rec)
    ...
    def cycle_f_pbigan(self, x, mask,  label, another_x, another_mask):
        ...
        ### generate feat
        if self.ae_weight > 0:
            feat, out = self.m_net(another_x * another_mask)
        else:
            feat = None
        
        _, _, f_rec, _, _ = self.f_pbigan(x * mask, mask, None, ae=False)    # generate m, x is fashion
        
        if self.ae_weight > 0:
            feat, out = self.f_net(f_rec)
        else:
            feat = None
        
        z_enc, z_gen, x_rec, x_gen, ae_loss = self.m_pbigan(another_x * another_mask, another_mask, feat, ae=True) # generate f
    ...
    def train_m_pbigan(self, x, mask, mask_gen, label, another_x, another_mask):
        ...
        if self.ae_weight > 0 and self.usefeat:
            ...
            feat, out = self.multi_cls(m_x_rec.detach(), f_x_rec.detach())
            self.m_pbigan.train()
        else:
            feat = None
            
        z_enc, z_gen, x_rec, x_gen, _ = self.m_pbigan(x * mask, mask, feat, ae=False)
        if self.args.keepobsv: 
...
#             self.loss_breakdown['cls_D'] += 0
        ...
        if self.critic_updates == self.n_critic:
            ...
            z_enc, z_gen, x_rec, x_gen, ae_loss = self.m_pbigan(x * mask, mask, feat)
            if self.args.keepobsv: 
                x_rec = x_rec * (1 - mask) + x * mask
            # GAN loss, G
            real_score = self.m_critic((x * mask, z_enc)).mean()
...
#             self.loss_breakdown['G'] += G_loss.item()
#             self.loss_breakdown['AE'] += ae_loss.item()
#             self.loss_breakdown['total'] += loss.item()
    ...
    def train_f_pbigan(self, x, mask, mask_gen, label, another_x, another_mask):
        ...
        z_enc, z_gen, x_rec, x_gen, _ = self.f_pbigan(x * mask, mask, feat, ae=False)
        if self.args.keepobsv: 
            x_rec = x_rec * (1 - mask) + x * mask
        ...
        if self.ae_weight > 0:
            if self.args.trclsori:
                gen_feat, gen_out = self.f_net(x)
            else:
                gen_feat, gen_out = self.f_net(x_rec)
            cls_loss  = F.nll_loss(F.log_softmax(gen_out, 1), label.cuda())
            cls_loss = min(10 * self.ae_weight, 1) * cls_loss
            self.optim_f_net.zero_grad()
            cls_loss.backward(retain_graph=True)
            self.optim_f_net.step()
#             self.loss_breakdown['cls_D'] += cls_loss.item()
#         else:
#             self.loss_breakdown['cls_D'] += 0
        ...
        z_enc, z_gen, x_rec, x_gen, ae_loss = self.f_pbigan(x * mask, mask, feat)
        if self.args.keepobsv: 
            x_rec = x_rec * (1 - mask) + x * mask
        # GAN loss, G
        real_score = self.f_critic((x * mask, z_enc)).mean()
...
#     parser.add_argument('--pretrain_path', type=str, default='/nfs/masi/gaor2/saved_file/MissClinic/MNIST2Fashion/cls/fashion_128dim/models/model_epoch_0134.pth') # 49 for mnist, 134 for fashion
    parser.add_argument('--pretrain_pbigan', type=str, default='/media/gaor2/8e7f6ccf-3585-4815-856e-80ce8754c5b5/saved_file/MissClinic/MissGAN/crossM_cls/20210212_10_22_trclsori_seed0/model/0099.pth')
    parser.add_argument('--plot-interval', type=int, default=10)
    parser.add_argument('--save-interval', type=int, default=10)
    parser.add_argument('--save_root', type=str, default='/media/gaor2/8e7f6ccf-3585-4815-856e-80ce8754c5b5/saved_file/MissClinic/MissGAN/crossM_cls')
    ...
    parser.add_argument('--f-block-len', type=int, default=10)
    parser.add_argument('--block-len-max', type=int, default=24)
    # option for indep:
    ...
    parser.add_argument('--save_foldname', default='20210212_10_26_trclsori_seed0')
