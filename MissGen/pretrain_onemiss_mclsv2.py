...
import pprint
import argparse
from masked_mnist import MNIST, MultiMNIST, MultiMNISTv2
...
def train_gaonet(args):
    ...
    data = MultiMNISTv2(m1root = args.mnist_path, m2root = args.fashion_path, block_len = args.block_len, train='train', transform2=trans_mnist, fix_seed = args.fix_seed)
    val_data = MultiMNISTv2(m1root = args.mnist_path, m2root = args.fashion_path, block_len = args.block_len, train='val', transform2=trans_mnist, fix_seed = args.fix_seed)
    
    
    
    data_loader = DataLoader(data, batch_size=args.batch_size, shuffle = True, drop_last=True)
    
    mask_loader = DataLoader(data, batch_size=args.batch_size,shuffle = True, drop_last=True)
    
    val_loader = DataLoader(data, batch_size=args.batch_size,shuffle = False, drop_last=True)

    # Evaluate the training progress using 2000 examples from the training data
    test_data = MultiMNISTv2(m1root = args.mnist_path, m2root = args.fashion_path, block_len = args.block_len, train='test',transform2=trans_mnist, fix_seed = args.fix_seed)
...
#                 loss_breakdown['cls_D'] += 0
            ...
            if critic_updates == n_critic:
                ...
                if ae_weight > 0 and args.useclassifier:
...
def main():
    ...
    parser.add_argument('--pretrain_pbigan', type=str, default='/nfs/masi/gaor2/saved_file/MissClinic/MNIST2Fashion/MissGAN/pretrain_onemiss_mcls/20210216_trvaltt/model/0099.pth')
    ...
    parser.add_argument('--block-len-max', type=int, default=14)
    # option for indep:
    ...
    parser.add_argument('--save_foldname', default='20210222_trvaltt_noclassifier')
    parser.add_argument('--latent', type=int, default=128)
    parser.add_argument('--aeloss', default='bce')   # mse|bce|smooth_l1|l1
    parser.add_argument('--fix_seed', default=False) 
    parser.add_argument('--useclassifier', default=False) 
