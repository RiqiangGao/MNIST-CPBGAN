...
#                         _, _, f_x_rec, _, _ = self.f_pbigan(test_x2 * test_mask2, test_mask2, None, ae=False)
                        ...
                        feat1, pred1, feat2, pred2, out = self.multi_cls(m_x_rec.detach(), test_x2)
                    ...
                    feat1, pred1_gen, feat2, pred2_gen, test_out = self.multi_cls(x_rec.detach(), test_x2)
                ...
                self.vis.plot(epoch + 10000, test_x2, test_mask2, bbox2, test_x2, test_x2)
    ...
    def train_epoch(self, epoch):
        ...
        for (xlist, masklist, _, label), (_, mask_genlist, _, _) in zip(self.data_loader, self.mask_loader):
            ...
            if self.args.trclsori:
                ...
                feat1, pred1, feat2, pred2, pred = self.multi_cls(m_rec, x2)   
...
#            loss_list.append(loss.data.cpu().numpy().tolist())
    ...
    def val_epoch(self, epoch, phase):
        ...
        for batch_idx, (xlist, masklist, _, label) in enumerate(loader):
            ...
            feat1, pred1_gen, feat2, pred2_gen, pred_gen = self.multi_cls(m_rec, x2)             # here should be careful
            ...
            feat1, pred1_gen, feat2, pred2_gen, pred_gen = self.multi_cls(m_rec, x2)
    ...
    def train_m_pbigan(self, x, mask, mask_gen, label, another_x, another_mask):
        ...
        if self.ae_weight > 0 and self.usefeat:
            ...
            feat1, pred1_gen, feat2, pred2_gen, pred_gen = self.multi_cls(m_x_rec.detach(), another_x)
            self.m_pbigan.train()
        else:
            feat2 = None
...
#                 p.requires_grad_(False) 
            ...
            if self.ae_weight > 0 and self.usefeat:
               # print ('feat is not noen')
                cls_loss  = 0.2 * F.nll_loss(F.log_softmax(pred_gen, 1), label.cuda()) + F.nll_loss(F.log_softmax(pred1_gen, 1), label.cuda())
...
def main():
    ...
    parser.add_argument('--f-block-len', type=int, default=28)
    parser.add_argument('--m-len-max', type=int, default=14)
    parser.add_argument('--f-len-max', type=int, default=None)
    # option for indep:
    ...
    parser.add_argument('--save_foldname', default='20210216_v2_single_pretrain')
