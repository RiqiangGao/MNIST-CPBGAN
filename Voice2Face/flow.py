"""Code adapted from https://github.com/altosaar/variational-autoencoder"""
import numpy as np
import torch
...
class FlowSequential(nn.Sequential):
    ...
    def forward(self, input, context=None):
        total_log_prob = torch.zeros_like(input)
...
class MaskedLinear(nn.Module):
    ...
    def forward(self, input, context=None):
        output = F.linear(input, self.mask * self.linear.weight,
                          self.linear.bias)
        if context is None:
            return output
        else:
...
class MADE(nn.Module):
    ...
    def __init__(self, num_input, num_output, num_hidden, num_context):
        ...
        modules.append(MaskedLinear(
            num_hidden, num_hidden, self._masks[1], context_features=None))
        modules.append(nn.ReLU())
        modules.append(MaskedLinear(
            num_hidden, num_output, self._masks[2], context_features=None))
    ...
    def _check_masks(self):
        ...
        if num_output == num_input:
            ...
            for submat in np.split(
                    final, indices_or_sections=num_output // num_input,
                    axis=1):
